...
class GroupService {
  ...
  /// Folosește o funcție PostgreSQL cu SECURITY DEFINER pentru a bypassa RLS
  Future<List<app_user.User>> getGroupMembersForAssignment(
    String groupId,
  ) async {
    try {
      // Obține ID-ul user-ului curent
      final currentUserId = _supabase.auth.currentUser?.id;
      if (currentUserId == null) {
        throw Exception('User not authenticated');
      }

      // Apelează funcția PostgreSQL care bypasează RLS
      final response = await _supabase.rpc(
        'get_group_members_for_assignment',
        params: {'p_group_id': groupId},
      );

      // Convertește răspunsul în listă de useri, excluzând user-ul curent
